...
Позволяет входить в веб-консоль и клиент по доменной учётной записи
(`user@domain.name` или `DOMAIN\User`). По умолчанию выключено.
Доменный пользователь создаётся автоматически при первом входе, его членство
в добавленных в систему AD-группах обновляется при каждом входе.
...
> При недоступности LDAP доменные входы отклоняются, локальные продолжают
> работать. Смена пароля доменных пользователей управляется каталогом и в
> админ-панели недоступна.
