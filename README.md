...
### Создание клиента (интеграция rdgen / rdgen-cli)

Раздел **«Создание клиента»** (`/clientgen`, только админ): список конфигураций
(название, ОС, версия клиента, сборка, дата сборки, автор), создание/редактирование/
дублирование/удаление конфигураций, кнопка **«Собрать»** (запускает `rdgen-cli` на
...
- `CLIENTGEN_DIR` — каталог артефактов (по умолчанию `/data/clientgen`).

Без `RDGEN_CLI`/`RDGEN_SERVER` сборка возвращает ошибку «rdgen-cli не настроен».
Настройка rdgen/rdgen-cli — по инструкциям
[rdgen setup](https://github.com/bryangerlach/rdgen/blob/master/setup.md) и
[rdgen-cli](https://github.com/AlekseyLapunov/rdgen-cli).
